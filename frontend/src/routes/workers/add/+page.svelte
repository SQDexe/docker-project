<script>
    ...
    let [name, surname, username, password, admin, phone_number, email] = ['', '', '', '', false, '', ''];
    
    async function add() {
        const token = $store.token;
        const response = await fetch(FetchBuilder.workers.add, {
            method: 'POST',
            mode: 'cors',
            headers: {
            ...
            body: JSON.stringify({ name, surname, username, password_hash: sha256(password), admin, phone_number, email })
            });
        ...
        <table>
            <tbody>
                ...
                <tr>
                    ...
                    <td><input type="password" name="password" id="password" required bind:value={password}/></td>
                </tr>
                <tr>
